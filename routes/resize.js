...
router.post('/', function(req, res) {
  ...
  //Grab width and height numbers from client
  let width = Number(req.body.width)
  let height = Number(req.body.height)

  if (width <= 0 || height <= 0 ) {
    return res.status(400).send('No negative numbers or zeros allowed.');

  } else if (isNaN(req.body.width) || isNaN(req.body.height)) {
    ...
    resizedImg.toBuffer((err, resizedBuffer) => {
        if (err) {
          console.log(err)
        } else {
          console.log('Buffer callback successful.')
          sharp(resizedBuffer).metadata()
